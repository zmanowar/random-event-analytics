...
public class GravediggerSolver implements SolverModuleComponent
{
	...
	public boolean isEnabled() {
		return client.isInInstancedRegion() && client.getMapRegions() != null
			&&  client.getMapRegions()[0] == GRAVEDIGGER_MAP_REGION_ID;
	}
	...
	private void setInventoryItems(Item[] items) {
		coffins = Arrays.stream(items).filter(item -> COFFIN_ITEM_ID_TO_NAME.containsKey(item.getId()))
			.map(item -> new Coffin(item, COFFIN_ITEM_ID_TO_NAME.get(item.getId())))
	...
	private void setHeadstoneNameByWidget() {
		...
		if (gravestoneWidget == null || gravestoneWidget.getModelId() == -1) return;
	...
	private boolean shouldRefreshCoffinItems() {
		return coffins == null || coffins.size() < COFFIN_ITEM_ID_TO_NAME.size() || coffins.stream()
			.anyMatch(coffin -> coffin.getName() == null || coffin.getName().equals(""));
	}
	...
	{
		if (event.getGroupId() != GRAVEDIGGER_WIDGET_GROUP_ID) return;
	...
	public void onMenuOptionClicked(MenuOptionClicked event) {
		...
		selectedHeadstone = headstones.stream().filter(headstone -> headstone.gameObject.getId() == event.getId()).findFirst().orElse(null);
	}
	...
	public void onItemContainerChanged(ItemContainerChanged event)
	{
		...
		if (container != client.getItemContainer(InventoryID.INVENTORY)) return;
	...
	public void onGameObjectSpawned(GameObjectSpawned event) {
		...
		if (!HEADSTONE_IDS.contains(gameObject.getId())) return;
